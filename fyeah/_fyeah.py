...
def f(template: Union[str, CompiledFstring]) -> str:
    ...
    elif isinstance(template, CodeType):
...
def compile(template: str) -> CompiledFstring:
    ...
    try:
        ...
        template = "f" + _triple_repr(template)
        compstring = builtins.compile(template, "<fyeah>", "eval")
    return compstring
...
def _triple_repr(original_str):
    ...
    while next_expression_start >= 0:
        next_expression_end = _find_closing_expression(original_repr, next_expression_start)
        ...
        next_expression_start = _find_opening_expression(original_repr, next_expression_end)
